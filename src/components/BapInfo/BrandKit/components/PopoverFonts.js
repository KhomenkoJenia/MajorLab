...
export const PopoverFonts = ({
	...
	useEffect(() => {
		...
		if (fontPicker) {
			fontPicker.classList.add('expanded');
		}
	}, [searchQuery]);
							...
							{FontPicker && (
								<FontPickerErrorHandler>
									<FontPicker
										...
										limit={800}
										filter={font => {
											return font.family.toLowerCase().startsWith(searchQuery.toLowerCase());
										}}
									/>
								</FontPickerErrorHandler>
							)}
...
const FontPickerErrorHandler = ({ children }) => {
	const [error, setError] = useState(null);

	// Обработка ошибки FontPicker
	const handleFontPickerError = error => {
		setError(error);
	};

	if (error) {
		// Здесь вы можете добавить обработчик ошибки, например, вывод сообщения об ошибке или логирование
		console.error('FontPicker Error:', error);
		return <p>Произошла ошибка при загрузке шрифтов.</p>;
	}
