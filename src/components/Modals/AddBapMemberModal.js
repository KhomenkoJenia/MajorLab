...
const AddBapMemberModal = ({
	...
	useEffect(() => {
		const sendInviteHandlerLink = async () => {
			const data = new FormData();
			data.append('email', `@email`);
			data.append('role', `roles[1]`);
			const resData = await sendInviteToBap(data, selectedBap?.bapId);
			if (resData.success) {
				const inviteTokenFromServer = resData.notification;
				setInviteUrl(`https://app.majorlabl.com/accept-invite?token=${inviteTokenFromServer}`);
			}
		};

		sendInviteHandlerLink();
	}, []);
	...
	const sendInviteHandler = async (
		...
		getToast('info', 'Loading', 'Sending invitation...');
